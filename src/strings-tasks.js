...
function removeFirstOccurrences(str, value) {
  ...
  const strStart = str.slice(0, indexStart);
  const strEnd = str.slice(indexEnd);
  return strStart.concat(strEnd);
}
...
function encodeToRot13(str) {
  ...
  const newArr = arr.map((item) => {
    if (!alph.includes(item)) {
      return item;
    }
    const index = alph.indexOf(item);
    return rot[index];
  });
  return newArr.join('');
}
